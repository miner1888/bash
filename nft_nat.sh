...
function enable_ping() {
    ...
    if [ "$policy" = "accept" ]; then
        ...
        if [ -n "$rule_handle" ]; then
            nft delete rule inet filter input handle "$rule_handle"
            echo -e "\e[32mPing已被允许。\e[0m"
        else
            echo -e "\e[33mPing已经是允许状态，无需操作。\e[0m"
        fi
