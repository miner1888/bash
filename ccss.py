...
# 配置 Web 操作日志
web_logger = logging.getLogger('web_operations')
web_handler = logging.FileHandler(os.path.join(LOG_DIR, 'web_operations.log'))
...
def log_request_info():
    web_logger.info(f"收到请求: 方法={request.method}, 路径={request.path}, 参数={request.args}, 表单={request.form}")
...
def index():
    if 'username' not in session:
        web_logger.info("未登录用户访问 /，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 访问主页")
    return render_template('index.html')

@app.route('/login', methods=['GET', 'POST'])
def login():
    web_logger.info(f"收到登录请求，方法: {request.method}")
    if request.method == 'POST':
        username = request.form['username']
        password = hashlib.sha256(request.form['password'].encode()).hexdigest()
        web_logger.info(f"登录尝试 - 用户名: {username}")
        with sqlite3.connect(TRADES_DB) as conn:
            c = conn.cursor()
            ...
            if result and result[0] == password:
                ...
                web_logger.info(f"用户 {username} 登录成功，重定向至主页")
                return redirect(url_for('index'))
        web_logger.warning(f"用户 {username} 登录失败：用户名或密码错误")
        flash('用户名或密码错误')
...
def logout():
    ...
    web_logger.info(f"用户 {username} 登出，重定向至登录页面")
    return redirect(url_for('login'))

@app.route('/settings', methods=['GET', 'POST'])
def settings():
    if 'username' not in session:
        web_logger.info("未登录用户访问 /settings，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 访问设置页面，方法: {request.method}")
    if not api_configs:
        web_logger.warning("无 API 配置，重定向至 API 设置页面")
        flash('请先添加 API 配置')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if 'save_and_run' in request.form:
                ...
                web_logger.info(f"添加并运行策略 {strat.symbol}")
                flash(f"已为 {request.form['symbol']} 添加并运行策略")
            else:
                web_logger.info(f"添加策略 {strat.symbol}（未运行）")
                flash(f"已为 {request.form['symbol']} 添加策略（默认停止运行）")
            return redirect(url_for('index'))
        except Exception as e:
            web_logger.error(f"保存策略错误: {e}", exc_info=True)
...
@app.route('/edit_strategy', methods=['GET', 'POST'])  # 修改路由，移除 <strat_id>
def edit_strategy():
    if 'username' not in session:
        web_logger.info("未登录用户访问 /edit_strategy，重定向至登录页面")
        return redirect(url_for('login'))
    
    if request.method == 'GET':
        strat_id = request.args.get('strat_id')  # 从 GET 参数获取 strat_id
        if not strat_id:
            web_logger.error("GET 请求缺少 strat_id 参数")
            flash('缺少策略 ID')
            return redirect(url_for('index'))
        web_logger.info(f"用户 {session['username']} 访问编辑策略页面，ID: {strat_id}, 方法: GET")
    else:  # POST
        strat_id = request.form.get('strat_id')  # 从表单数据获取 strat_id
        if not strat_id:
            web_logger.error("POST 请求缺少 strat_id 参数")
            flash('缺少策略 ID')
            return redirect(url_for('index'))
        web_logger.info(f"用户 {session['username']} 访问编辑策略页面，ID: {strat_id}, 方法: POST")

    strat_index = next((i for i, s in enumerate(strategies) if str(id(s)) == strat_id), None)
    if strat_index is None:
        web_logger.error(f"策略 {strat_id} 不存在")
        flash('策略不存在')
        return redirect(url_for('index'))
    ...
    if request.method == 'POST':
        web_logger.info(f"收到编辑策略 POST 请求，ID: {strat_id}, Symbol: {strat.symbol}, 表单数据: {request.form}")
        try:
            if 'save_and_restart' not in request.form:
                web_logger.warning("表单中未找到 'save_and_restart'，可能是按钮未正确触发")
                flash('提交失败：未检测到保存请求')
                return redirect(url_for('edit_strategy') + f'?strat_id={strat_id}')
            ...
            if was_running:
                web_logger.info(f"停止运行中的策略 {strat.symbol}")
                strat.stop()
                time.sleep(3)
                if id(strat) in running_strategies:
                    del running_strategies[id(strat)]
                    web_logger.info(f"从 running_strategies 移除 {strat.symbol}")
            ...
            web_logger.info(f"策略 {new_strat.symbol} 更新至 strategies 列表，位置: {strat_index}")

            # 保存到文件
            save_strategies()
            web_logger.info(f"策略保存完成，文件: {STRATEGIES_FILE}")

            # 如果之前运行，则重启新策略
            if was_running:
                web_logger.info(f"重新启动策略 {new_strat.symbol}")
                executor.submit(new_strat.run)
                flash(f"已修改并重启 {new_strat.symbol} 的策略")
            else:
                flash(f"已修改 {new_strat.symbol} 的策略（未运行）")

            web_logger.info(f"编辑策略 {new_strat.symbol} 成功，重定向至主页")
            return redirect(url_for('index'))
        except Exception as e:
            web_logger.error(f"编辑策略 {strat_id} 失败: {str(e)}", exc_info=True)
            flash(f"编辑策略失败: {str(e)}")
            return redirect(url_for('edit_strategy') + f'?strat_id={strat_id}')
    
    action_url = url_for('edit_strategy') + f'?strat_id={strat_id}'
    web_logger.info(f"渲染编辑页面，策略: {strat.symbol}, 生成的 action URL: {action_url}")
    return render_template('edit_strategy.html', strat=strat, api_configs=api_configs, usdt_pairs=usdt_pairs, strat_id=strat_id)
@app.route('/api_settings', methods=['GET', 'POST'])
def api_settings():
    if 'username' not in session:
        web_logger.info("未登录用户访问 /api_settings，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 访问 API 设置页面，方法: {request.method}")
    if request.method == 'POST':
        web_logger.info(f"收到 API 设置请求，表单数据: {request.form}")
        if 'test_connection' in request.form:
            try:
                ...
                web_logger.info(f"API 连接测试成功: {exchange_name} {'模拟账户' if is_test_mode else '实盘账户'}")
                flash(f"API 连接测试成功: {exchange_name} {'模拟账户' if is_test_mode else '实盘账户'}")
            except Exception as e:
                web_logger.error(f"API 测试连接失败: {e}")
...
def delete_api(api_id):
    if 'username' not in session:
        web_logger.info("未登录用户访问 /delete_api，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 访问删除 API 页面，API ID: {api_id}, 方法: {request.method}")
    if request.method == 'GET':
        api = next((a for a in api_configs if f"{a['exchange']}_{a['api_key']}" == api_id), None)
        if not api:
            web_logger.error(f"API 配置 {api_id} 不存在")
            flash('API 配置不存在')
            return redirect(url_for('api_settings'))
        return render_template('confirm_delete_api.html', api=api)
    if request.method == 'POST':
        web_logger.info(f"收到删除 API 请求，API ID: {api_id}")
        try:
            api = next((a for a in api_configs if f"{a['exchange']}_{a['api_key']}" == api_id), None)
            if api:
                ...
                if key in pairs_cache:
                    del pairs_cache[key]
                save_api_configs()
                web_logger.info(f"API 配置 {api['name']} 已删除")
            ...
            web_logger.error(f"删除 API 配置错误: {e}")
...
def trades():
    if 'username' not in session:
        web_logger.info("未登录用户访问 /trades，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 访问交易记录页面，方法: {request.method}")
    ...
    if start_date and end_date:
        try:
            ...
            if start_dt > end_dt:
                ...
                web_logger.debug(f"交易记录查询: start_time={start_time}, end_time={end_time}")
        except ValueError:
            web_logger.warning("日期格式错误")
    ...
    with sqlite3.connect(TRADES_DB) as conn:
        ...
        web_logger.debug(f"总记录数: {total_records}")
    ...
    with sqlite3.connect(TRADES_DB) as conn:
        ...
        web_logger.debug(f"查询结果记录数: {len(trade_records)}")
...
def profits():
    if 'username' not in session:
        web_logger.info("未登录用户访问 /profits，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 访问利润统计页面，方法: {request.method}")
    ...
    if start_date and end_date:
        try:
            ...
            if start_dt > end_dt:
                ...
                web_logger.debug(f"利润统计查询: start_time={start_time}, end_time={end_time}")
        except ValueError:
            web_logger.warning("日期格式错误")
            flash('日期格式错误，请使用 YYYY-MM-DD')
            start_date, end_date = ''
    ...
    try:
        with sqlite3.connect(TRADES_DB) as conn:
            ...
            web_logger.debug(f"利润统计查询结果: {daily_profits}")
    except Exception as e:
        web_logger.error(f"利润统计查询错误: {e}")
        daily_profits = []
...
def assets():
    if 'username' not in session:
        web_logger.info("未登录用户访问 /assets，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 访问资产查询页面，方法: {request.method}")
    if request.method == 'POST':
        web_logger.info(f"收到资产查询请求，表单数据: {request.form}")
        api_id = request.form['api_id']
        api = next((a for a in api_configs if f"{a['exchange']}_{a['api_key']}" == api_id), None)
        if not api:
            web_logger.error(f"API 配置 {api_id} 不存在")
...
def start_strategy(strat_id):
    if 'username' not in session:
        web_logger.info("未登录用户访问 /start，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 请求启动策略，ID: {strat_id}")
    strat = next((s for s in strategies if str(id(s)) == strat_id), None)
    if strat and not strat.running:
        executor.submit(strat.run)
        web_logger.info(f"策略 {strat.symbol} 已启动")
        flash(f"已启动 {strat.symbol} 的策略")
    elif strat and strat.running:
        web_logger.warning(f"策略 {strat.symbol} 已在运行")
...
def pause_strategy(strat_id):
    if 'username' not in session:
        web_logger.info("未登录用户访问 /pause，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 请求暂停策略，ID: {strat_id}")
    strat = next((s for s in strategies if str(id(s)) == strat_id), None)
    if strat and strat.running:
        strat.stop()
        time.sleep(1)
        web_logger.info(f"策略 {strat.symbol} 已暂停")
        flash(f"已暂停 {strat.symbol} 的策略")
    elif strat and not strat.running:
        web_logger.warning(f"策略 {strat.symbol} 已停止")
...
def delete_strategy(strat_id):
    if 'username' not in session:
        web_logger.info("未登录用户访问 /delete，重定向至登录页面")
        return redirect(url_for('login'))
    web_logger.info(f"用户 {session['username']} 访问删除策略页面，ID: {strat_id}, 方法: {request.method}")
    strat = next((s for s in strategies if str(id(s)) == strat_id), None)
    if not strat:
        web_logger.error(f"策略 {strat_id} 不存在")
        flash('策略不存在')
    ...
    if request.method == 'POST':
        web_logger.info(f"收到删除策略请求，ID: {strat_id}")
        try:
            retain_positions = request.form.get('retain_positions') == 'yes'
            if strat.running:
                strat.stop()
                time.sleep(1)
                web_logger.info(f"策略 {strat.symbol} 已停止运行")
            if not retain_positions and strat.positions:
                ...
                web_logger.info(f"策略 {strat.symbol} 持仓已卖出")
            strategies.remove(strat)
            if id(strat) in running_strategies:
                del running_strategies[id(strat)]
            save_strategies()
            update_websocket_subscriptions()
            web_logger.info(f"策略 {strat.symbol} 删除完成，持仓{'保留' if retain_positions else '已卖出'}")
            flash(f"已删除 {strat.symbol} 的策略，持仓{'保留' if retain_positions else '已卖出'}")
            return redirect(url_for('index'))
        except Exception as e:
            web_logger.error(f"删除策略 {strat_id} 错误: {e}")
            flash(f"删除策略失败: {str(e)}")
            return redirect(url_for('index'))

@app.route('/data')
def get_data():
    web_logger.info(f"用户 {session.get('username', '未知用户')} 请求获取数据")
    try:
        status = []
        ...
        """
        web_logger.info("数据获取成功，返回策略状态")
        return jsonify({'strategies': strategies_html, 'summary': summary_html})
    except Exception as e:
        web_logger.error(f"获取数据错误: {e}")
        return jsonify({'strategies': '', 'summary': ''})

# 测试 POST 请求
@app.route('/test_post', methods=['POST'])
def test_post():
    web_logger.info(f"收到测试 POST 请求，数据: {request.form}")
    return jsonify({'status': 'success', 'data': request.form.to_dict()})
            ...
            <div class="card-body">
                <form method="post">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" name="max_add_positions" class="form-control" value="{{ last_params.get('max_add_positions', 2) }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">补仓下跌%</label>
                        <input type="number" name="add_drop_percent" class="form-control" value="{{ last_params.get('add_drop_percent', 0.3) }}" step="0.01" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">补仓反弹%</label>
...
<body class="bg-light">
    <div class="container mt-4">
        ...
        <div class="card">
            <div class="card-body">
                <form method="POST" action="{{ url_for('edit_strategy') }}">
                    <input type="hidden" name="strat_id" value="{{ strat_id }}">
                    <div class="mb-3">
                        <label class="form-label">选择 API</label>
                        <select name="api_id" class="form-select" required>
                            {% for api in api_configs %}
                    ...
                    <div class="mb-3">
                        <label class="form-label">最大补仓次数</label>
                        <input type="number" name="max_add_positions" class="form-control" value="{{ strat.max_add_positions }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">补仓下跌%</label>
                    ...
                    <button type="submit" name="save_and_restart" class="btn btn-primary">保存并重启</button>
                    <a href="{{ url_for('index') }}" class="btn btn-secondary">返回</a>
                </form>
            </div>
        </div>
